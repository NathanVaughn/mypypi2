...
def lookup_package_version_filename(repository_slug: str, filename: str) -> PackageVersionFilename | None:
    """
    Lookup a PackageVersionFilename object given the filename
    """
    return db.session.execute(db.select(PackageVersionFilename).join(PackageVersionFilename.repository).where(
...
def lookup_package_update(repository_slug: str, package: str) -> PackageUpdate | None:
    ...
    """
    return db.session.execute(db.select(PackageUpdate).join(PackageUpdate.repository).where(
...
def get_package_version_filenames(repository_slug: str, package: str) -> list[PackageVersionFilename]:
    ...
    return db.session.execute(
        ...
        ).join(PackageVersionFilename.repository).where(PackageVersionFilename.package == package, Repository.slug == repository_slug)
...
def update_package_last_updated(repository_slug: str, package: str) -> None:
    ...
    package_update = db.session.execute(
        db.select(PackageUpdate).join(PackageUpdate.repository).where(
            PackageUpdate.package == package, Repository.slug == repository_slug)
    ).scalar_one_or_none()
